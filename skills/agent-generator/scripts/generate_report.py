...
def generate_html(data: dict, auto_refresh: bool = False, skill_name: str = "") -> str:
    ...
    has_test_results = bool(test_queries)
...
"""
    ...
    if has_test_results and best_test_score:
        best_score_line = (
            f"<p><strong>Best Test Score:</strong> {best_test_score} | "
            f"<strong>Best Train Score:</strong> {best_train_score or 'N/A'}</p>"
        )
        data_split_line = (
        ...
        best_score_line = f"<p><strong>Best Score:</strong> {best_train_score or data.get('best_score', 'N/A')} (train)</p>"
        data_split_line = (
            f"<p><strong>Iterations:</strong> {data.get('iterations_run', 0)} | "
            f"<strong>Train:</strong> {data.get('train_size', '?')}</p>"
        )
...
""")
    ...
    for h in history:
        ...
        test_class = score_class(test_correct, test_runs) if has_test_results else ""
